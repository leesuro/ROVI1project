...
/// Read the image
	...
	for (cnti = 1; cnti < 51; cnti++) {
		...
		if (cnti < 10)
			sstm
					<< "/home/pyc/workspace/ROVI1project/res/markers/marker_thinline_hard/marker_thinline_hard_0"
					<< cnti << ".png";
		else
			sstm
					<< "/home/pyc/workspace/ROVI1project/res/markers/marker_thinline_hard/marker_thinline_hard_"
					<< cnti << ".png";
...
/// Show the result
	...
	if (crossings.size() > 0) {
		...
		vector<Point2f> crossings_new;
		vector<size_t> ignored;
		Point2f actual;
		float distance;
		/*for (size_t i = 0; i < crossings.size(); i++) {
		 actual=crossings[i];
		 crossings_new.push_back(actual);
		 for (size_t j = i; j < crossings.size(); j++) {
		 distance = pointDistance(actual,crossings[j]);

		 //cout<<"Distance: "<<distance<<endl;
		 if (distance<18.0){
		 cout<<"Distance is smaller"<<endl;
		 }
		 else{crossings_new.push_back(crossings[j]);
		...
		imshow("LinesH", thinHough);
								waitKey();
		for (size_t i = 0; i < crossings.size(); i++){
			circle(thinHough, crossings[i], 1, Scalar(0, 0, 255), 3);
		}

			imshow("LinesH", thinHough);
						waitKey();

						thinHough = thinHoughEdit.clone();
						for (size_t i = 0; i < crossings.size(); i++){
								circle(thinHough, crossings[i], 1, Scalar(0, 0, 255), 3);
							}
		...
		for (size_t i = 0; i < crossings.size(); i++) {
			...
			//cout << ignored[k] << " ";
			if (ignored.size() > 3)
				//crossings.erase(crossings.begin() + ignored[k]);
				crossings_new.push_back(actual);
			//}
			ignored.clear();
			cout << endl;
		}
		centerMass = crossings[0];

		for (size_t i = 1; i < crossings_new.size(); i++) {
		...
		centerMass.x = centerMass.x / crossings_new.size();
		centerMass.y = centerMass.y / crossings_new.size();
