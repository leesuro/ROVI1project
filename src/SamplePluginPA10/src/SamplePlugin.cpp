...
#elif (SPEED==2)
const string motionFilePath = userPath
		+ "ROVI1project/src/SamplePluginPA10/motions/MarkerMotionFast.txt";
#endif
...
#elif (TRACKING==4)
const string markerPath = userPath
		+ "ROVI1project/src/SamplePluginPA10/markers/Marker3.ppm";
#endif

const QString iconPath = QString::fromStdString(userPath)
		+ "ROVI1project/src/SamplePluginPA10/src/pa_icon.png";
const string workcellPath = userPath
		+ "ROVI1project/res/PA10WorkCell/ScenePA10RoVi1.wc.xml";
const string backgroundPath = userPath
		+ "ROVI1project/src/SamplePluginPA10/backgrounds/color1.ppm";
...
void SamplePlugin::open(WorkCell* workcell) {
	...
	if (_wc != NULL) {
		...
		if (textureFrame != NULL)
			getRobWorkStudio()->getWorkCellScene()->addRender("TextureImage",
					_textureRender, textureFrame);

		// Add the background render to this workcell if there is a frame for texture
		Frame* bgFrame = _wc->findFrame("Background");
		if (bgFrame != NULL)
			getRobWorkStudio()->getWorkCellScene()->addRender("BackgroundImage",
					_bgRender, bgFrame);
		...
		if (cameraFrame != NULL) {
			if (cameraFrame->getPropertyMap().has("Camera")) {
				...
				double fovy;
				int width, height;
				std::string camParam = cameraFrame->getPropertyMap().get<
						std::string>("Camera");
				std::istringstream iss(camParam, std::istringstream::in);
				iss >> fovy >> width >> height;
				// Create a frame grabber
				_framegrabber = new GLFrameGrabber(width, height, fovy);
				SceneViewer::Ptr gldrawer =
						getRobWorkStudio()->getView()->getSceneViewer();
	...
	if (TRACKING == 4)
		_spinBox->setValue(700);
	if (TRACKING == 2)
		_spinBox->setValue(900);
...
void SamplePlugin::close() {
	...
	Frame* textureFrame = _wc->findFrame("MarkerTexture");
	if (textureFrame != NULL)
		getRobWorkStudio()->getWorkCellScene()->removeDrawable("TextureImage",
				textureFrame);
	// Remove the background render
	Frame* bgFrame = _wc->findFrame("Background");
	if (bgFrame != NULL)
		getRobWorkStudio()->getWorkCellScene()->removeDrawable(
				"BackgroundImage", bgFrame);
...
Mat SamplePlugin::getImageAndShow() {
	...
	// Show in QLabel
	QImage imageToShow(image.data, image.cols, image.rows, image.step,
			QImage::Format_RGB888);
	_label->setPixmap(
			QPixmap::fromImage(imageToShow).scaled(600, 800,
					Qt::KeepAspectRatio));
...
void SamplePlugin::writeData() {
	...
	if (TRACKING == 4)
		tracking = "Corny";
	if (SPEED == 0)
		experiment = tracking + "Slow";
	if (SPEED == 1)
		experiment = tracking + "Medium";
	if (SPEED == 2)
		experiment = tracking + "Fast";

	//Q Robot
	ofstream qRobotFile;
	qRobotFile.open((qRobotPath + experiment + ".tex").c_str());
	qRobotFile.precision(3);
	qRobotFile.setf(std::ios::fixed);
	latexPlotQ(qRobotFile, ("qRobot" + experiment + "Plot"));
	qRobotFile.close();

	//Speed
	ofstream speedFile;
	speedFile.open((speedPath + experiment + ".tex").c_str());
	speedFile.precision(3);
	speedFile.setf(std::ios::fixed);
	latexPlotSpeed(speedFile, ("speed" + experiment + "Plot"));
	speedFile.close();

	//Camera Pose
	ofstream cameraPoseFile;
	cameraPoseFile.open((cameraPosePath + experiment + ".tex").c_str());
	cameraPoseFile.precision(3);
	cameraPoseFile.setf(std::ios::fixed);
	latexPlotCameraPose(cameraPoseFile, ("cameraPose" + experiment + "Plot"));
	cameraPoseFile.close();

	//Error
	ofstream errorFile;
	errorFile.open((errorPath + tracking + ".tex").c_str());
	errorFile.precision(3);
	errorFile.setf(std::ios::fixed);
	latexPlotError(errorFile, ("error" + tracking + "Plot"));
	errorFile.close();

	ofstream errorXFile;
	errorXFile.open((errorPath + experiment + "X.txt").c_str());
	errorXFile.precision(3);
	errorXFile.setf(std::ios::fixed);
	for (unsigned int i = 0; i < _errorPoseVec.size() - 1; i++)
		errorXFile << "				(" << _sumDeltaT[i] / 1000 << ","
				<< _errorPoseVec[i + 1](0) << ")\n";
	errorXFile.close();

	ofstream errorYFile;
	errorYFile.open((errorPath + experiment + "Y.txt").c_str());
	errorYFile.precision(3);
	errorYFile.setf(std::ios::fixed);
	for (unsigned int i = 0; i < _errorPoseVec.size() - 1; i++)
		errorYFile << "				(" << _sumDeltaT[i] / 1000 << ","
				<< _errorPoseVec[i + 1](1) << ")\n";
	errorYFile.close();
}
...
void SamplePlugin::computeError() {
	Vector3D<> Pdiff = ((_markerPoseVec.end() - 2)->P()
			- (_markerPoseVec.end() - 1)->P()) * 823 * 2;
...
void SamplePlugin::calculateAverageTime() {
	double total = 0;
	for (unsigned int i = 0; i < _featureExtractionTime.size(); i++)
		total += _featureExtractionTime[i];
...
void SamplePlugin::calculateDeltaT() {
	...
	float deltaT;
	if (TRACKING != 0)
		deltaT = (_spinBox->value() - *(_featureExtractionTime.end() - 1));
	else
	...
	if (_firstTime)
		_sumDeltaT.push_back((long) deltaT);
	else
		_sumDeltaT.push_back((long) deltaT + *(_sumDeltaT.end() - 1));
}
...
void SamplePlugin::checkVelocityLimits(Q & dqToCheck) {
	for (unsigned char joint = 0; joint < 7; joint++) {
		if (*(_deltaT.end() - 1) == 0
				|| abs(dqToCheck(joint)) * 1000 / *(_deltaT.end() - 1)
						> abs(_device->getVelocityLimits()(joint))) {
			if (abs(dqToCheck(joint)) * 1000 / *(_deltaT.end() - 1)
					> abs(_device->getVelocityLimits()(joint)))
				log().info() << abs(_device->getVelocityLimits()(0)) << "\n";
			if (dqToCheck(joint) > 0)
				dqToCheck(joint) = abs(_device->getVelocityLimits()(joint))
						* *(_deltaT.end() - 1) / 1000;
			if (dqToCheck(joint) < 0)
				dqToCheck(joint) = -abs(_device->getVelocityLimits()(joint))
						* *(_deltaT.end() - 1) / 1000;
...
Q SamplePlugin::getdQ(Mat & image) {
	...
	if (TRACKING == 0) {
		Vector3D<> Pt0 = (inverse(
				_device->worldTbase(_state)
						* _device->baseTframe(_wc->findFrame("Camera"), _state))
				* _wc->findFrame("Marker")->getTransform(_state))
				* Vector3D<>(0, 0, 0);
		Vector3D<> Pt1 = (inverse(
				_device->worldTbase(_state)
						* _device->baseTframe(_wc->findFrame("Camera"), _state))
				* _wc->findFrame("Marker")->getTransform(_state))
				* Vector3D<>(0.1, 0, 0);
		Vector3D<> Pt2 = (inverse(
				_device->worldTbase(_state)
						* _device->baseTframe(_wc->findFrame("Camera"), _state))
				* _wc->findFrame("Marker")->getTransform(_state))
				* Vector3D<>(0, 0.1, 0);
		u[0] = Pt0(0) * 823 * 2;
		v[0] = Pt0(1) * 823 * 2;
		if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
			u[1] = Pt1(0) * 823 * 2;
		if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
			v[1] = Pt1(1) * 823 * 2;
		if (NUMBER_OF_POINTS == 3)
	...
	//Get points from OpenCV algorithms
		...
		if (TRACKING == 1)
			point = colorDetection(image);
		if (TRACKING == 2)
			point = linesHDetection(image);
		if (TRACKING == 4)
			point = cornyDetection(image);
	...
	for (unsigned char i = 0; i < NUMBER_OF_POINTS; i++) {
		imageJacobian(i * 2, 0) = f / z;
		imageJacobian(i * 2, 1) = 0;
		imageJacobian(i * 2, 2) = -u[i] / z;
		imageJacobian(i * 2, 3) = -u[i] * v[i] / f;
		imageJacobian(i * 2, 4) = (f * f + u[i] * u[i]) / f;
		imageJacobian(i * 2, 5) = -v[i];
		imageJacobian(i * 2 + 1, 0) = 0;
		imageJacobian(i * 2 + 1, 1) = f / z;
		imageJacobian(i * 2 + 1, 2) = -v[i] / z;
		imageJacobian(i * 2 + 1, 3) = -(f * f + u[i] * u[i]) / f;
		imageJacobian(i * 2 + 1, 4) = u[i] * v[i] / f;
		imageJacobian(i * 2 + 1, 5) = u[i];
	}

	//As the d(u,v) is not referenced to the base, we need to adapt it with Sq
	Matrix3d R_device_T =
			(_device->baseTframe(_wc->findFrame("Camera"), _state).R().e()).transpose();

	MatrixXd Sq(6, 6);
	for (unsigned char row = 0; row < 6; row++) {
		for (unsigned char col = 0; col < 6; col++) {
			if (row < 3 && col < 3)
				Sq(row, col) = R_device_T(row, col);
			else if (row > 2 && col > 2)
				Sq(row, col) = R_device_T(row - 3, col - 3);
			else
				Sq(row, col) = 0;
		}
	}
	...
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		dudv(2, 0) = u[1] - _previousPoints[1][0];
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		dudv(3, 0) = v[1] - _previousPoints[1][1];
	if (NUMBER_OF_POINTS == 3)
		dudv(4, 0) = u[2] - _previousPoints[2][0];
	if (NUMBER_OF_POINTS == 3)
		dudv(5, 0) = v[2] - _previousPoints[2][1];

	//Store the actual point
	_previousPoints[0][0] = u[0];
	_previousPoints[0][1] = v[0];
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		_previousPoints[1][0] = u[1];
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		_previousPoints[1][1] = v[1];
	if (NUMBER_OF_POINTS == 3)
		_previousPoints[2][0] = u[2];
	if (NUMBER_OF_POINTS == 3)
		_previousPoints[2][1] = v[2];

	//And calculate dq
	MatrixXd dq_aux(7, 1);
	dq_aux = Zimage.transpose() * (Zimage * Zimage.transpose()).inverse()
			* dudv; //0.97 is the max
	...
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		_previousdUdV[1][0] = dudv(2, 0);
	if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
		_previousdUdV[1][1] = dudv(3, 0);
	if (NUMBER_OF_POINTS == 3)
		_previousdUdV[2][0] = dudv(4, 0);
	if (NUMBER_OF_POINTS == 3)
		_previousdUdV[2][1] = dudv(5, 0);
...
void SamplePlugin::loop() {
	if (_motionFile.is_open()) {
		...
		MovableFrame* markerFrame = static_cast<MovableFrame*>(_wc->findFrame(
				"Marker"));
		...
		if (getline(_motionFile, line)) {
			...
			if (_firstTime) {
				...
				if (TRACKING == 0) {
					...
					if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
						_previousPoints[1][0] = Pt1(0) * 823 * 2;
					if (NUMBER_OF_POINTS == 2 || NUMBER_OF_POINTS == 3)
						_previousPoints[1][1] = Pt1(1) * 823 * 2;
					if (NUMBER_OF_POINTS == 3)
						_previousPoints[2][0] = Pt2(0) * 823 * 2;
					if (NUMBER_OF_POINTS == 3)
						_previousPoints[2][1] = Pt2(1) * 823 * 2;
				}
			...
			_cameraPoseVec.push_back(
					_device->worldTbase(_state)
							* _device->baseTframe(_wc->findFrame("Camera"),
									_state));
...
Point2f SamplePlugin::cornyDetection(Mat & img_input) {
	...
	Mat descriptors_marker, descriptors_scene;


	cvtColor(img_input, img_scene, CV_RGB2GRAY);

	Mat img_marker = imread(markerPath, CV_LOAD_IMAGE_GRAYSCALE);
	...
	extractor.compute(img_marker, keypoints_object, descriptors_marker);
...
Point2f SamplePlugin::colorDetection(Mat & img_input) {
	...
	for (unsigned char icounter = 0; icounter < 2; icounter++) {
		...
		if (icounter == 0)
			inRange(imHSV, Scalar(108, 253, 240), Scalar(120, 255, 255),
					im_thresh); //Threshold the image red circle
		else
			inRange(imHSV, Scalar(0, 253, 240), Scalar(12, 255, 255),
...
bool SamplePlugin::isPossible(Vec2f & line_1, Vec2f & line_2,
...
Point2f SamplePlugin::calculateLineCrossing(Vec2f & line_1, Vec2f & line_2) {
	...
	float denumerator = (line1_pt1.x - line_1_pt2.x)
			* (line2_pt1.y - line2_pt2.y)
			- (line1_pt1.y - line_1_pt2.y) * (line2_pt1.x - line2_pt2.x);
	intersect.x = ((line1_pt1.x * line_1_pt2.y - line1_pt1.y * line_1_pt2.x)
			* (line2_pt1.x - line2_pt2.x)
			- (line1_pt1.x - line_1_pt2.x)
					* (line2_pt1.x * line2_pt2.y - line2_pt1.y * line2_pt2.x))
			/ denumerator;
...
void SamplePlugin::latexPlotSpeed(ofstream & file, const string & name) {
	file << "\\providecommand{\\" << name << "}{\n" << "\\begin{figure}[!ht]\n"
			...
			<< "			\\addplot [color=Turquoise, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](0) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$1}\n" << "\n"
			<< "			\\addplot [color=Orchid, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](1) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$2}\n" << "\n"
			<< "			\\addplot [color=YellowGreen, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](2) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$3}\n" << "\n"
			<< "			\\addplot [color=WildStrawberry, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](3) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$4}\n" << "\n"
			<< "			\\addplot [color=Goldenrod, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](4) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$5}\n" << "\n"
			<< "			\\addplot [color=BlueGreen, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](5) << ")\n";
	file << "			};\n" << "			\\addlegendentry{$\\dot q$6}\n" << "\n"
			<< "			\\addplot [color=CadetBlue, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _speedVec.size(); i++)
		file << "				(" << i << ", " << _speedVec[i](6) << ")\n";
...
void SamplePlugin::latexPlotCameraPose(ofstream & file, const string & name) {
	file << "\\providecommand{\\" << name << "}{\n" << "\\begin{figure}[!ht]\n"
			...
			<< "			\\addplot [color=Turquoise, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << _cameraPoseVec[i].P()(0) << ")\n";
	file << "			};\n" << "			\\addlegendentry{x}\n" << "\n"
			<< "			\\addplot [color=Orchid, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << _cameraPoseVec[i].P()(1) << ")\n";
	file << "			};\n" << "			\\addlegendentry{y}\n" << "\n"
			<< "			\\addplot [color=YellowGreen, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << _cameraPoseVec[i].P()(2) << ")\n";
	file << "			};\n" << "			\\addlegendentry{z}\n" << "\n"
			<< "			\\addplot [color=WildStrawberry, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << RPY<>(_cameraPoseVec[i].R())(0)
				<< ")\n";
	file << "			};\n" << "			\\addlegendentry{R}\n" << "\n"
			<< "			\\addplot [color=Goldenrod, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << RPY<>(_cameraPoseVec[i].R())(1)
				<< ")\n";
	file << "			};\n" << "			\\addlegendentry{P}\n" << "\n"
			<< "			\\addplot [color=BlueGreen, mark=o] coordinates {\n";
	for (unsigned int i = 0; i < _qRobotVec.size(); i++)
		file << "				(" << i << ", " << RPY<>(_cameraPoseVec[i].R())(2)
				<< ")\n";
	file << "			};\n" << "			\\addlegendentry{Y}\n" << "\n" << "		\\end{axis}\n"
			<< "	\\end{tikzpicture}\n"
